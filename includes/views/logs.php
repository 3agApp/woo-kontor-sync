...
<div class="wssc-wrap">
    <div class="wssc-header">
        <div class="wssc-header-left">
            <h1><?php esc_html_e('Sync Logs', 'woo-kontor-sync'); ?></h1>
            <p class="wssc-subtitle"><?php esc_html_e('View history of all sync operations', 'woo-kontor-sync'); ?></p>
        </div>
        <div class="wssc-header-right">
            <button type="button" id="wssc-clear-logs" class="wssc-btn wssc-btn-danger">
                <span class="dashicons dashicons-trash"></span>
                <?php esc_html_e('Clear Logs', 'woo-kontor-sync'); ?>
            </button>
        </div>
    </div>
    ...
    <div class="wssc-section">
        <div class="wssc-filters">
            <form method="get" action="">
                <input type="hidden" name="page" value="woo-kontor-sync-logs">
                ...
                <div class="wssc-filter-group">
                    <label><?php esc_html_e('Status', 'woo-kontor-sync'); ?></label>
                    <select name="status" class="wssc-select">
                        <option value=""><?php esc_html_e('All Statuses', 'woo-kontor-sync'); ?></option>
                        <option value="success" <?php selected($status_filter, 'success'); ?>><?php esc_html_e('Success', 'woo-kontor-sync'); ?></option>
                        <option value="error" <?php selected($status_filter, 'error'); ?>><?php esc_html_e('Error', 'woo-kontor-sync'); ?></option>
                        <option value="warning" <?php selected($status_filter, 'warning'); ?>><?php esc_html_e('Warning', 'woo-kontor-sync'); ?></option>
                    </select>
                </div>

                <button type="submit" class="wssc-btn wssc-btn-secondary">
                    <span class="dashicons dashicons-filter"></span>
                    <?php esc_html_e('Filter', 'woo-kontor-sync'); ?>
                </button>

                <?php if ($type_filter || $status_filter): ?>
                    <a href="<?php echo esc_url(WKS_Admin::get_page_url('logs')); ?>" class="wssc-btn wssc-btn-ghost">
                        <?php esc_html_e('Reset', 'woo-kontor-sync'); ?>
                    </a>
                <?php endif; ?>
            </form>
        </div>
    ...
    <div class="wssc-section">
        <?php if (empty($logs)): ?>
            <div class="wssc-empty-state">
                ...
                <p><?php esc_html_e('No logs found.', 'woo-kontor-sync'); ?></p>
            </div>
        <?php else: ?>
            <div class="wssc-table-wrap">
                <table class="wssc-table">
                    <thead>
                        <tr>
                            <th class="wssc-col-status"><?php esc_html_e('Status', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-type"><?php esc_html_e('Type', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-trigger"><?php esc_html_e('Trigger', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-message"><?php esc_html_e('Message', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-stats"><?php esc_html_e('Stats', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-duration"><?php esc_html_e('Duration', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-date"><?php esc_html_e('Date', 'woo-kontor-sync'); ?></th>
                            <th class="wssc-col-actions"><?php esc_html_e('Actions', 'woo-kontor-sync'); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($logs as $log): ?>
                            <tr class="wssc-log-row wssc-log-<?php echo esc_attr($log->status); ?>">
                                ...
                                <td class="wssc-col-stats">
                                    <?php if (!empty($log->stats) && is_array($log->stats)): ?>
                                        <div class="wssc-mini-stats">
                                            <?php if (isset($log->stats['created'])): ?>
                                                <span class="wssc-mini-stat wssc-mini-stat-success" title="<?php esc_attr_e('Created', 'woo-kontor-sync'); ?>">
                                                    <span class="dashicons dashicons-plus-alt"></span>
                                                    <?php echo esc_html($log->stats['created']); ?>
                                                </span>
                                            <?php endif; ?>
                                            <?php if (isset($log->stats['updated'])): ?>
                                                <span class="wssc-mini-stat" title="<?php esc_attr_e('Updated', 'woo-kontor-sync'); ?>">
                                                    <span class="dashicons dashicons-yes"></span>
                                                    <?php echo esc_html($log->stats['updated']); ?>
                                                </span>
                                            <?php endif; ?>
                                            <?php if (isset($log->stats['skipped'])): ?>
                                                <span class="wssc-mini-stat" title="<?php esc_attr_e('Skipped', 'woo-kontor-sync'); ?>">
                                                    <span class="dashicons dashicons-minus"></span>
                                                    <?php echo esc_html($log->stats['skipped']); ?>
                                                </span>
                                            <?php endif; ?>
                                            <?php if (isset($log->stats['errors']) && $log->stats['errors'] > 0): ?>
                                                <span class="wssc-mini-stat wssc-mini-stat-error" title="<?php esc_attr_e('Errors', 'woo-kontor-sync'); ?>">
                                ...
                                <td class="wssc-col-actions">
                                    <button type="button" class="wssc-btn wssc-btn-icon wssc-view-log" data-log-id="<?php echo esc_attr($log->id); ?>" title="<?php esc_attr_e('View Details', 'woo-kontor-sync'); ?>">
            ...
            <?php if ($total_pages > 1): ?>
                <div class="wssc-pagination">
                    <div class="wssc-pagination-info">
                        <?php
                        printf(
                            esc_html__('Showing %1$d-%2$d of %3$d logs', 'woo-kontor-sync'),
                            $offset + 1,
                    ...
                    <div class="wssc-pagination-links">
                        <?php if ($page > 1): ?>
                            <a href="<?php echo esc_url(add_query_arg('paged', $page - 1)); ?>" class="wssc-btn wssc-btn-sm wssc-btn-secondary">
                                &larr; <?php esc_html_e('Previous', 'woo-kontor-sync'); ?>
                            </a>
                        <?php endif; ?>
                        ...
                        <span class="wssc-page-num">
                            <?php
                            printf(
                                esc_html__('Page %1$d of %2$d', 'woo-kontor-sync'),
                                $page,
                                $total_pages
                            );
                            ?>
                        ...
                        <?php if ($page < $total_pages): ?>
                            <a href="<?php echo esc_url(add_query_arg('paged', $page + 1)); ?>" class="wssc-btn wssc-btn-sm wssc-btn-secondary">
                                <?php esc_html_e('Next', 'woo-kontor-sync'); ?> &rarr;
                            </a>
                        <?php endif; ?>
                    </div>
                </div>
...
<div id="wssc-log-modal" class="wssc-modal">
    <div class="wssc-modal-content wssc-modal-lg">
        <div class="wssc-modal-header">
            <h3><?php esc_html_e('Log Details', 'woo-kontor-sync'); ?></h3>
